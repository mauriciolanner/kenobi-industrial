<template>
  <jet-action-section>
    ...
    <template #description>
      Confira onde seu login está em uso.
    </template>
    ...
    <template #content>
      ...
      <div>
        Se necessário você pode deslogar de outro lugar que está logado com seu perfil
      </div>
          ...
          <div class="ms-2">
            ...
            <div>
              <div class="small font-weight-lighter text-muted">
                ...
                <span class="text-success font-weight-bold" v-if="session.is_current_device">Essa sessão</span>
                <span v-else>Ultimo acesso {{ session.last_active }}</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <jet-dialog-modal id="confirmingLogoutModal">
        ...
        <template #content>
          Por favor, digite sua senha para confirmar que você gostaria de sair de suas outras sessões do navegador em
          todos os
          seus dispositivos.
