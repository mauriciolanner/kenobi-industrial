    ...
    <app-layout title="Etiquetas Impressas">
        ...
        <div class="card-body card-tabelas bg-white shadow-sm border-bottom rounded-top table-responsive">
            <div class="row mb-3">
                ...
                <div class="col-md-2">
                    <button type="button" v-on:click="buscar" class="btn btn-success mt-30">Buscar</button>
                    <button type="button" style="margin-left:20px" v-on:click="limpar"
                        class="btn btn-danger mt-30">Limpar</button>
                </div>
            </div>
            <div class="row">
                <div class="col-md-12">
                    <table class="table table-striped table-padrao">
                        ...
                        <tbody>
                            <tr v-for="consulta in consultas">
                                <th>{{ consulta.NumSeq }}</th>
                                <td>{{ consulta.OrdemProducao }}</td>
                                <td>{{ consulta.Turno }}</td>
                                <td>{{ consulta.DataIni }}</td>
                                <td>{{ consulta.HoraIni }}</td>
                                ...
                                <td>{{ parseInt(consulta.Falta) }}</td>
                                <td>{{ consulta.Operador }}</td>
                                <td>{{ parseInt(consulta.Via) }}</td>
                                <td><a :href="asset + 'impressas/pdf/' + consulta.Recno + '/' + consulta.OrdemProducao + '/' + consulta.NumSeq + '/ApontamentoImpresso'"
                                        target="blank" class="btn btn-info"><i class="bi bi-printer"></i></a></td>
                                <td><button v-if="btnbuscar" type="button" data-bs-toggle="modal"
                                        :data-bs-target="'#exampleModal' + consulta.NumSeq" class="btn btn-info"><i
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h5 class="modal-title" id="exampleModalLabel">NumSeq:
                                                        {{ consulta.NumSeq }}</h5>
                                                </div>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-12">
                                                            <table class="table table-striped table-padrao">
                                                                <thead>
                                                                    <tr>
                                                                        <th scope="col">Saldo PR</th>
                                                                    </tr>
                                                                </thead>
                                                                <tbody>
                                                                    <tr v-for="saldoPR in saldopr">
                                                                        <th>{{ parseInt(saldoPR.SaldoPR) }}</th>
                                                                    </tr>
                                                                </tbody>
                                                            </table>
                                                        </div>
                                                    ...
                                                    <div class="row">
                                                        <div class="col-md-12">
                                                            <table class="table table-striped table-padrao">
                                                                <thead>
                                                                    <tr>
                                                                        <th scope="col">Saldo LO</th>
                                                                    </tr>
                                                                </thead>
                                                                <tbody>
                                                                    <tr v-for="saldoLO in saldolo">
                                                                        <th>{{ parseInt(saldoLO.SaldoLO) }}</th>
                                                                    </tr>
                                                                </tbody>
                                                            </table>
                                                        </div>
                                                            ...
                                                            <div class="col-md-12">
                                                                <table class="table table-striped table-padrao">
                                                                    <thead>
                                                                        <tr>
                                                                            <th scope="col">OP</th>
                                                                            <th scope="col">Qtd da OP</th>
                                                                            <th scope="col">Apontado</th>
                                                                    ...
                                                                    <tbody>
                                                                        <tr v-for="consulta in consulta2">
                                                                            ...
                                                                            <td>{{ parseInt(consulta.QtdOrdem) }}</td>
                                                                            <td>{{ parseInt(consulta.Apontado) }}</td>
                                                                            <td>{{ parseInt(consulta.Falta) }}</td>
                                                                        </tr>
                                                                    </tbody>
                                                                </table>
    ...
    methods: {
        buscar() {
            this.$inertia.get(route('consulta.impressas'),
                { busca: this.buscador, dataini: this.dataInicio, datafinal: this.dataFim },
                { preserveState: true },
                this.btnbuscar = true
            );
        },
